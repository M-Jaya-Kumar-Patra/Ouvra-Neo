...
import {
  createSplitRecord,
  extractInvoiceData,
} from "@/lib/actions/split.actions";
...
export function BillSplitter({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    ...
    const amountsAreEmpty = friends.every(
  ...
  const handleQRSuccess = (decodedText: string) => {
    if (decodedText.includes("upi://pay")) {
      ...
      // CRITICAL: Capture the Merchant Code if it's a business QR
      const mc = urlParams.get("mc");
      setMerchantCode(mc || ""); // Will be empty for personal QRs
  ...
  const addFriend = () =>
    setFriends([...friends, { name: "", amount: "", upiId: "", userId: "" }]);
  ...
  upiQuery += `&mc=${merchantCode}`; // ✅ only then
          ...
          if (isAndroid) {
           ...
           window.location.href = `intent://upi/pay?${upiQuery}#Intent;scheme=upi;end`;
           } else {
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          <div className="flex gap-2 items-center">
            <div className="flex-1 bg-blue-500/10 border border-blue-500/30 rounded-xl px-4 py-2.5 text-xs text-blue-400 font-bold">
              Your Share
            </div>
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] text-blue-500/50">
                ₹
              </span>
              <input
                value={myAmount}
                readOnly
                className="w-24 md:w-28 bg-blue-500/5 border border-blue-500/20 rounded-xl pl-6 pr-4 py-2.5 text-xs text-blue-400 font-mono font-bold outline-none"
              />
            </div>
            <div className="w-8" />
          </div>
            ...
            {friends.map((friend, idx) => (
              ...
              >
                ...
                <div className="relative shrink-0">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] text-zinc-500">
                    ₹
                  </span>
                  <input
                    type="number"
        ...
        {total && Math.abs(remaining) > 0.01 && (
          <div
            className={`text-[10px] font-bold px-3 py-1 rounded-full mx-auto w-fit ${remaining > 0 ? "bg-amber-500/10 text-amber-500" : "bg-rose-500/10 text-rose-500"}`}
          >
            {remaining > 0
        ...
        <div className="flex gap-2 pt-2">
          <button
            onClick={addFriend}
            className="flex-1 py-3 bg-zinc-800 hover:bg-zinc-700 text-white text-xs font-bold rounded-xl flex items-center justify-center gap-2"
          >
            <Plus className="h-3 w-3" /> Add Friend
          </button>
          <button
            onClick={splitEqual}
            className="flex-1 py-3 bg-zinc-800 hover:bg-zinc-700 text-white text-xs font-bold rounded-xl"
          >
            Split Equally
          </button>
        </div>
      ...
      {isQRScannerOpen && (
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-md flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-3xl p-6 relative">
            ...
            <div
              id="qr-reader"
              className="overflow-hidden rounded-2xl border-2 border-indigo-500 bg-black aspect-square w-full"
            ></div>
