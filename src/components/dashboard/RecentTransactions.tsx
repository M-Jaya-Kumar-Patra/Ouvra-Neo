...
export function RecentTransactions({ 
  ...
  fullWidth 
    ? "w-full flex-1 rounded-[2.5rem] border-t border-zinc-800/50" 
    : "max-h-[420px] rounded-3xl"
)}>
      <CardHeader className="flex flex-row items-center justify-between pb-6 px-8 pt-8">
        <div className="space-y-1">
          <CardTitle className={cn("font-bold", fullWidth ? "text-3xl" : "text-xl")}>
            {fullWidth ? "All Transactions" : "Recent Activity"}
          </CardTitle>
          <p className="text-sm text-zinc-500">
            {fullWidth ? "A detailed record of your wealth movement." : "Your latest transactions."}
          </p>
        </div>
        {!fullWidth && (
      ...
      <div className={cn("pb-4", fullWidth ? "px-8" : "px-6")}>
        <TransactionSearch />
      </div>

      <CardContent className={cn(
        "flex-1 overflow-y-auto scrollbar-hide pb-8", 
        fullWidth ? "px-8" : "px-2"
      )}>
  ...
  key={t._id.toString()} 
  className={cn(
    "group flex items-center justify-between px-6 py-5 rounded-2xl transition-all",
    "hover:bg-zinc-800/60 border-b border-zinc-800/30 last:border-none", // Subtle divider
    fullWidth ? "mx-2" : ""
  )}
>
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-medium px-2 py-0.5 rounded-md bg-zinc-800 text-zinc-400">
                      {t.category}
                    </span>
                    {t.type === 'owed_to_me' && (
                       <span className="text-[10px] uppercase tracking-widest text-blue-400 font-bold">Split Transaction</span>
                    )}
                  </div>
                </div>
                ...
                <div className="text-right">
                  <div className={cn(
                    "text-xl font-bold transition-all duration-300 group-hover:scale-110",
