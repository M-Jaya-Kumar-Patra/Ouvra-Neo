...
async function DashboardContent({
  ...
  // Logic for filtering
  const filteredTransactions = (allTransactions as unknown as TransactionDoc[])
    .filter((t) => {
    ...
    <CardContent>
      ...
      <p className={`text-xs flex items-center gap-1 mt-2 ${isPositive ? "text-emerald-500" : "text-rose-500"}`}>
        ...
        <div className="">
          ...
          >
            <WalletOverview lent={splitSummary.totalLent} />
          </Suspense>
        </div>
      </div>
      ...
      <div className="w-full">
        <Suspense
          fallback={
            <div className="h-32 w-full bg-zinc-900/50 animate-pulse rounded-3xl border border-zinc-800" />
          }
        >
          <AIInsightCard />
        </Suspense>
...
function QuickLink({
  ...
  return (
    <Link
      href={href}
      className="flex items-center justify-between p-4 rounded-2xl bg-zinc-900/50 border border-zinc-800 hover:border-zinc-700 hover:bg-zinc-800 transition-all group"
    >
